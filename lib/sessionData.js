...
function _sessionData(variable, data) {
	...
	if (variable && data) {
		...
		if (variable == 'breadcrumb') {
			...
			if (data=='back') { 
				console.log('%s datas[variable].length',srcName,datas[variable].length)
				if (datas[variable].length < 2 ) {
					return datas[variable][0]
				} else {
					datas[variable].pop();
					return datas[variable].pop();
				}
			} else {
				if (datas[variable] && (datas[variable][datas[variable].length-1] == data)){
					return;
				}
			}
		} 
